...
- These button clicks also a .css file include between `light-theme.css` and `dark-theme.css`, which are just short lists of CSS variables (see the [Look and Feel section](#section-7)).
- I add an extra "OS Theme" button which clears any user preference, reverting the page to the OS preferred theme.
- Finally, I poll the user / OS preferences every second to apply any updates.
